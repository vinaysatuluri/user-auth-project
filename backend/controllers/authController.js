...
export const forgotPasswordHandler = async (req, res) => {
  ...
  try {
    ...
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    await pool.query('UPDATE users SET otp = ?, otp_expiry = NOW() + INTERVAL 15 MINUTE WHERE email = ?', [otp, email]);
...
export const resetPasswordHandler = async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE users SET password_hash = ?, otp = NULL, otp_expiry = NULL WHERE email = ?', [hashedPassword, email]);
